...
///                                number of points, xmin, xmax, number of toys, mwp, use number counting)
///
...
*/
   ...
   calc.AnalyzeResult( r, calculatorType, testStatType, useCLs, npoints, mwp, infile, outfile );
...
RooStats::HypoTestInvTool::AnalyzeResult( HypoTestInverterResult * r,
   ...
   TFile *output = TFile::Open(filename, "UPDATE");
   TNtuple *results = (TNtuple*) output->Get("upper_limit");
   if (!results){
     TNtuple *tnt = new TNtuple("upper_limit", "upper_limit", "mwp:median:plus_1sig:plus_2sig:minus_1sig:minus_2sig");
     results = tnt;
   }
   results->Fill((float_t) mwp, (float_t) r->GetExpectedUpperLimit(0), (float_t) r->GetExpectedUpperLimit(1), (float_t) r->GetExpectedUpperLimit(2), (float_t) r->GetExpectedUpperLimit(-1), (float_t) r->GetExpectedUpperLimit(-2));
   results->Write("upper_limit",TObject::kOverwrite);
   output->Close();
